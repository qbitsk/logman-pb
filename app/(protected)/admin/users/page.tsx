...
export default function AdminUsersPage() {
  ...
  async function submitCreate(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      const err = await res.json().catch(() => ({ error: t.adminUsers.createError }));
      setCreateError(err?.error ?? t.adminUsers.createError);
    }
    setCreating(false);
  }
  ...
  async function submitEdit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      const err = await res.json().catch(() => ({ error: t.adminUsers.saveError }));
      setEditError(err?.error ?? t.adminUsers.saveError);
    }
    setSaving(false);
  }

  async function deleteUser(user: User) {
    if (!confirm(t.adminUsers.deleteConfirm(user.name))) return;
    const res = await fetch(`/api/admin/users/${user.id}`, { method: "DELETE" });
    if (res.ok || res.status === 204) {
      setUsers((prev) => prev.filter((u) => u.id !== user.id));
    } else {
      const err = await res.json().catch(() => ({ error: "Failed to delete" }));
      alert(err?.error ?? t.adminUsers.deleteError);
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-brand-950 dark:text-white">{t.adminUsers.title}</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-lg font-semibold text-brand-950 dark:text-white">{t.adminUsers.newUserModal}</h2>
            ...
            <form onSubmit={submitCreate} className="px-6 py-5 space-y-4">
              <div>
                <label className="label">{t.adminUsers.name}</label>
              ...
              <div>
                <label className="label">{t.adminUsers.password}</label>
              ...
              <div className="flex justify-end gap-3 pt-1">
                ...
                <button type="submit" disabled={creating} className="btn-primary">{creating ? t.workerProductionForm.saving : t.adminUsers.newUserModal}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {editTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-lg font-semibold text-brand-950 dark:text-white">{t.adminUsers.editUserModal}</h2>
            ...
            <form onSubmit={submitEdit} className="px-6 py-5 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-1">
                <button type="button" onClick={() => setEditTarget(null)} className="btn-secondary">{t.common.cancel}</button>
                <button type="submit" disabled={saving} className="btn-primary">{saving ? t.workerProductionForm.saving : t.common.save}</button>
              </div>
            </form>
          </div>
        </div>
